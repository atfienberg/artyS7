...
class uartClass:
    ...
    def exe_cmd(self,logging=None,s_act=None,h_act=None,act=None,s_adr=None,h_adr=None,adr=None,h_length=None,length=None,h_data=None,data=None):
        self.cc.parse_cmd(logging,s_act,h_act,act,s_adr,h_adr,adr,h_length,length,h_data,data)
        if(self.cc.cmd['act'] == self.cc.S_ACT_DICT['srd']):
            ...
            if(self.cc.cmd['ok']): 
                logging.debug("uartClass: DEBUG: single read adr = 0x%04x: 0x%04x" % (self.cc.cmd['adr'],self.cc.cmd['data']))            
            else:
                logging.warning("uartClass: ERROR: Checksum wrong for single read 0x%04x: 0x%04x" % (self.cc.cmd['adr'],self.cc.cmd['data']))            
        ...
        # ATF -- burst read and write
            ...
            if(self.cc.cmd['ok']): 
                logging.debug("uartClass: DEBUG: burst read adr = 0x%04x: 0x%04x" % (self.cc.cmd['adr'],self.cc.cmd['data']))            
            else:
                logging.warning("uartClass: ERROR: Checksum wrong for burst read 0x%04x: 0x%04x" % (self.cc.cmd['adr'],self.cc.cmd['data']))            
