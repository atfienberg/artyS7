...
class cmdClass():
    ...
    # Constants
    S_ACT_DICT = {'swr'                            : 0x0001, # single write
                  'srd'                            : 0x0002, # single read
                  'bwr'                            : 0x8001, # burst write
                  'brd'                            : 0x8002} # burst read
    ...
    def gse_cmd_str(self):
        return ('cmd: act = ' + hex(self.cmd['act']) + ', len = ' + hex(self.cmd['len']) + ', adr = ' + hex(self.cmd['adr'])  + ', data = ' + hex(self.cmd['data']) + ', crc16 = ' + hex(self.cmd['crc16']) + ', ok = ' + str(self.cmd['ok']))  

    
    def parse_cmd(self,logging,s_act,h_act,act,s_adr,h_adr,adr,h_length,length,h_data,data):
        # Decode the data
        if(data != None):
            self.cmd['data'] = data
        elif(h_data != None):
        ...
        if(adr != None):
            ...
            logging.warning("comClass: ERROR: Must specify one of s_adr, h_adr, or adr! Exiting!")
            exit(-1)
        # Decode the action
        if(act != None):
            self.cmd['act'] = act
            ...
            logging.warning("comClass: ERROR: Must specify either s_act or act! Exiting!")
            exit(-1)        
